...
| 38 | 2026-07-10 | Wheel-glide loop scrolls with `behavior:'instant'` (fixes #36's implementation) | With OS animation effects ON, plain `scrollTo` obeyed CSS `scroll-behavior:smooth` — each glide frame restarted a browser-animated scroll and froze the page. Scroll feel is now identical whether the OS reduced-motion setting is on or off | Arnav (bug report), Claude fix |
